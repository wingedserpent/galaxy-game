...
Shader "Custom/Refractive surface"
{
	Properties 
	{
		...
		[Header (Distortion)]
		_DistortAmt ( "Distortion amount", Range (0.0, 1.0) ) = 0.3
		[Normal] _BumpMap ( "Normal distortion map", 2D ) = "bump" {}
	...
	SubShader 
	{
		...
		void surf ( Input i, inout SurfaceOutputStandard s ) 
		{
			...
			float2 distort = bump * pow (_DistortAmt * DISTORTION_MAX + 1, 2.0);
			i.screenPos.xy += (distort * _GrabTexture_TexelSize.xy) * i.screenPos.z;
