...
public class PlayerInputManager : MonoBehaviour {
		...
		if (Input.GetMouseButtonUp(0) && (currentTargetingReference == null || isTargetingValid)) {
			...
			if (currentTargetingResourceCost == 0 || clientGameManager.MyPlayer.resources >= currentTargetingResourceCost) {
				if (currentTargetingReference == null) {
					...
					IssueAttackLocationCommand(new List<string>() { currentTargetingEntity.uniqueId }, currentTargetingObject.transform.position);
			...
			if (Physics.Raycast(ray, out hit, Mathf.Infinity, LayerManager.Instance.groundMask)) {
				...
				if (currentTargetingReference == null) {
					...
					if (currentTargetingEntity != null) {
						isValidPlacement = Vector3.Distance(currentTargetingEntity.transform.position, currentTargetingObject.transform.position) <= currentTargetingEntity.equippedWeapon.weaponData.range;
					} else {
		...
		if (Input.GetButtonDown("Build")) {
			...
			foreach (InputCommand buildCommand in buildCommands) {
				if (Input.GetKeyDown(buildCommand.key)) {
					//check resource cost
					Structure structureRef = clientEntityManager.GetEntityReference(buildCommand.command) as Structure;
					if (structureRef != null) {
						if (clientGameManager.MyPlayer.resources >= structureRef.resourceCost) {
							DeselectAll();
							SetTargeting(Instantiate<GameObject>(structureRef.targetingPrefab), structureRef.resourceCost, structureRef, null);
						}
					} else {
	...
	private void IssueAbilityCommand(List<string> entityIds, string abilityTypeId) {
		if (entityIds.Count > 0) {
			EntityCommand abilityCommand = new EntityCommand(CommandType.ABILITY, entityIds);
			abilityCommand.abilityTypeId = abilityTypeId;
